...
Exit(6)\n""")
        if(x=="1"):
            ...
            curr.execute("SELECT Title,Creationyear From Painting join Artist on ArtistID=ID where (Name||' '||COALESCE(Surname,''))='" +name+"';")
            data=curr.fetchall()
            print("RESULTS:\n")
            if (len(data)==0):
                print("No results based on given parameters")
            ...
            os.system('cls')
            curr.execute("SELECT ArtPeriod_name,count(ArtPeriod_name) as c from Painting GROUP by ArtPeriod_name ORDER BY c ASC;")
            data=curr.fetchall()
            print("RESULTS:\n")
            for i in data:
                print(i[0],i[1])
