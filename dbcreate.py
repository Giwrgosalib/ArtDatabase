    ...
    if(x=="1"):
        ...
        try:
            ...
            if(res==None):
                ...
                if(res==None):
                    conn.execute("INSERT INTO Associate_Gallery(Name) VALUES(?)",(gall,))
                conn.execute("INSERT INTO Takes (BorrowerName,LoanStart,LoanEnd,artid) VALUES(?,?,?,?)",(gall,datetime.date(startyear,startmonth,startday),datetime.date(endyear,endmonth,endday),artid))
                conn.commit()
        except:
            print("Wrong Value")
    elif(x=="4"):
...
7)Delete Displayed Painting\n""")
    if(x=="1"):
        ...
        os.system('cls')
        z=input("Desired Exhibition Subject to delete:\n")
        try:
