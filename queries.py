...
Show List Of Paintings That were Displayed In Past Exhibitions (7)
Exit(8)\n""")
        if(x=="1"):
            os.system('cls')
...
ORDER by Title ASC;""")
            ...
            os.system('cls')
            curr.execute("""SELECT DISTINCT Title,ArtPeriod_name,Name||' '||coalesce(Surname,'') as CreatorName FROM 
Painting NATURAL join Loan NATURAL join IsDisplayed join exhibition on Exhibition.ID=IsDisplayed.ExhibitionId JOIN Artist on ArtistID=Artist.ID
WHERE strftime("%Y-%m-%d",'now')>strftime("%Y-%m-%d",Exhibition.EndDate)""")
            data=curr.fetchall()
            style="{:<35}{:15}{:<10}"	
            print("RESULTS:\n")
            if (len(data)==0):
                ...
                print(style.format("Title","Art Period","Creator"))
                for i in data:
