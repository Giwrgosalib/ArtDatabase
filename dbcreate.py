...
7)Delete Displayed Painting\n""")
    if(x=="1"):
        ...
        try:
            curr.execute(" Delete from Artist where (Name||' '||COALESCE(Surname,'') = ?;",(z,))
            conn.commit()
        except:
            print("Wrong value")
    elif(x=="3"):
        ...
        try:
            curr.execute(" Delete from Exhibition where Subject = ?;",(z,))
            conn.commit()
        except:
            print("Wrong value")
    elif(x=="4"):
        ...
        z=input("Desired Painting title to delete:\n")
        try:
            curr.execute("Delete from Loan where artid in (Loan natural join Painting where Title = ?);",(z,))
            conn.commit()
        except:
            print("Wrong value")
    elif(x=="5"):
        ...
        try:
            curr.execute(" Delete from  Takes WHERE artid in(SELECT artid FROM Painting natural join Owned natural join Takes where Title= ?);",(z,))
            conn.commit()
        except:
            print("Wrong value")
    elif(x=="6"):
        ...
        try:
            curr.execute(" Delete from  Takes WHERE artid in(SELECT artid FROM Painting natural join Loan natural join IsDisplayed where Title= ?);",(z,))
            conn.commit()
        except:
            print("Wrong value")
